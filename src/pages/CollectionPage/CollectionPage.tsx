...
const missingLabels: Readonly<Record<string, string>> = {
  'reviewed-entities': 'Reviewed historical entity membership', territories: 'Reviewed territory coverage', places: 'Reviewed places', people: 'Reviewed people', events: 'Reviewed events', journeys: 'Reviewed journeys', 'source-review': 'Historical source review',
}
  ...
  return (
    <article className="collection-page">
      ...
      <header className="collection-page__header">
        <div><p className="eyebrow">Content collection</p><h1>{collection.defaultName}</h1><div className="collection-badges"><span>{collection.editorialStatus}</span><span>{collection.completeness.replaceAll('-', ' ')}</span><span>{collection.coverageStatus} coverage</span></div><p>{collection.description}</p></div>
        <Link className="button button--primary" to={createCollectionRecommendedMapHref(collection)}>Open recommended map</Link>
      </header>
      ...
      <section className="collection-section collection-section--notice" aria-label="Foundation preview note"><h2>Foundation preview</h2><p>{collection.editorialNotes}</p><p>{collection.coverage.note}</p></section>
      ...
      <section className="collection-section"><h2>Current linked content</h2><p>{counts.total} synthetic foundation demonstration records: {counts.active} active at {formatHistoricalYear(collection.recommendedStartYear)} ({counts.activeMapped} mapped and {counts.activeUnmapped} unmapped), {counts.mapped} with map context across their full periods, and {counts.unmapped} without map context.</p><p>These records exercise the collection experience only. They are not genuine historical members of Alexander’s world.</p>{members.length === 0 ? <p>Reviewed entity membership will arrive in F17.</p> : (Object.entries(groups) as [SelectedEntityType, typeof members][]).map(([type, items]) => items.length === 0 ? null : <section className="collection-member-group" key={type}><h3>{groupLabels[type]} ({items.length})</h3><ul>{items.map((item) => { const mapHref = searchIndex === null ? null : createEntityMapHref(searchIndex, item.type, item.id, collection.recommendedStartYear, baseState); return <li key={item.id}><div><strong>{item.name}</strong><span>{item.subtype.replaceAll('-', ' ')} · {item.mapped ? 'mapped' : 'unmapped'} · synthetic demonstration</span></div><div className="collection-member-actions"><Link to={entityPagePath(item.type, item.id)}>Open page</Link>{mapHref === null ? null : <Link to={mapHref}>View on map</Link>}</div></li> })}</ul></section>)}</section>
      <section className="collection-section"><h2>Content still missing</h2><ul>{collection.missingContent.map((item) => <li key={item}>{missingLabels[item] ?? item.replaceAll('-', ' ')}</li>)}</ul><p>F17 will add reviewed historical content. F16 does not make historical source claims.</p></section>
      <section className="collection-section"><h2>Editorial provenance</h2><p>No collection-level historical source is claimed in this foundation milestone. The linked source reference supports only the synthetic fixtures used for demonstration.</p></section>
